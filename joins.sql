...
SELECT posts.title AS "Post Title", comments.*
    FROM posts RIGHT JOIN comments ON posts.id = comments.post_id
...
SELECT posts.title AS "post_title", users.first_name, users.last_name, comments.body AS "comment_body"
    FROM posts LEFT JOIN users ON posts.user_id = users.id LEFT JOIN comments ON posts.id = comments.post_id
...
SELECT users.first_name, users.last_name, comments.body AS "comment_body"
    FROM comments LEFT JOIN users ON comments.user_id = users.id LEFT JOIN posts ON comments.post_id = posts.id
